...
import { ChevronRight, ChevronLeft, Settings2, CheckSquare, SlidersHorizontal, Hash, Rocket } from "lucide-react";
...
interface AdvancedQuizProps {
  onStartQuiz: (selection: { subjectId: string; difficulty?: string; subtopics?: string[]; questionCount?: number }) => void;
}

export function AdvancedQuiz({ onStartQuiz }: AdvancedQuizProps) {
  ...
  const handleLaunch = () => {
    ...
    const diffStr = difficulty <= 2 ? "easy" : difficulty <= 5 ? "medium" : difficulty <= 8 ? "hard" : "hard";
    onStartQuiz({ subjectId: selectedSubject, subtopics: selectedSubtopics, difficulty: diffStr, questionCount });
  };
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#4f46e5] text-white hover:bg-[#4338ca] transition-colors cursor-pointer"
          >
            <Rocket size={16} />
            Launch Quiz
          </button>
