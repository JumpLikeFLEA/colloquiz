  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 space-y-6">
      ...
      <div className="flex items-center gap-2">
        <TabLink href="/app/admin/review?tab=pending" active={tab === "pending"}>
          Pending
        </TabLink>
        <TabLink href="/app/admin/review?tab=reported" active={tab === "reported"}>
          Reported
...
async function PendingTab({ pageParam }: { pageParam?: string }) {
  ...
  if (requestedPage > totalPages && total > 0) {
    redirect(`/app/admin/review?tab=pending&page=${totalPages}`);
  }
