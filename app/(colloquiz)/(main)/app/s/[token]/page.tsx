...
// to /signup?next=/app/s/[token] (or /login), and after sign-up / confirm / OAuth
// the AuthScreen redirectTo guard lands them back here, now signed in, to play.
...
export default async function SharePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const meta = [label, preview.question_count ? pluralize(preview.question_count, "question") : null]
    .filter(Boolean)
    .join(" · ");
  const next = encodeURIComponent(`/app/s/${token}`);
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      <div className={card}>
        ...
        {user ? (
          <Link
            href={`/app/quiz/${preview.snapshot_quiz_id}`}
