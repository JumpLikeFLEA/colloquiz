...
export function MyQuizzesView({
  ...
  async function copyShareLink(token: string) {
    try {
      await navigator.clipboard.writeText(`${window.location.origin}/app/s/${token}`);
            ...
            <div className="flex flex-col gap-2">
              {created.map((q) => (
                <div key={q.id} className="flex items-center gap-3 p-4 rounded-2xl border border-border bg-card">
                  ...
                  <Link
                    href={`/app/my-quizzes/builder?id=${q.id}`}
                    title="Edit"
      ...
      <DialogContent>
        ...
        {students.length === 0 ? (
          <div className="py-4 text-center text-sm text-muted-foreground">
            ...
            <Link href="/app/students" className="text-brand-text font-medium hover:underline" onClick={onClose}>
              Invite a student
            </Link>
            .
          </div>
