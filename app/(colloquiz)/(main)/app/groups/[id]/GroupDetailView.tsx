...
export function GroupDetailView({
  ...
  const inviteUrl =
    token && typeof window !== "undefined"
      ? `${window.location.origin}/app/groups/join/${token}`
      : "";
  ...
  async function removeMember(uid: string) {
    ...
    if (uid === userId) router.push("/app/groups");
    router.refresh();
  }
  ...
  async function createQuiz() {
    ...
    setQuizTitle("");
    router.push(`/app/groups/${group.id}/builder?quiz=${data.quizId}`);
  }
  ...
  return (
    <div className="max-w-3xl mx-auto py-6 space-y-8">
      ...
      {pendingCount > 0 && (
        <Link
          href={`/app/groups/${group.id}/review`}
          ...
          <div className="flex flex-col gap-2">
            {quizzes.map((q) => (
              ...
              >
                ...
                <Link
                  href={`/app/groups/${group.id}/builder?quiz=${q.id}`}
