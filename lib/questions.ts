...
export async function getPendingQuestions(
  ...
  const { data, error, count } = await supabase
    ...
    .select("*", { count: "exact" })
    .eq("status", "pending")
    // The admin queue moderates the public pool ('shared') only. Group
    // questions are also 'pending' while they await their own group's peer
    // review but are visibility='group', so they stay excluded until a group
    // promotes one to 'shared'. Private questions are created 'approved'
    // (buildQuestionRows), so this filter hides no existing row.
